...
public class QAReviewNotifyCustomerMC extends MCLoginDetails {
     @Test
    public void QAReviewerNotifyCustomer() throws InterruptedException {
        Thread.sleep(2000);
    ...
    private void   methodToDoQAReviewNotifyCustomer() throws InterruptedException {
        int count = 0;
...
//        String DepartmentCode = properties.getProperty("CHG_CONTROL_NO");
        ...
        isRecordSelected = selectRecdQAReviewNotifyCustomer(MCNumber, isRecordSelected, count);
        Thread.sleep(8000);
        if (isRecordSelected) {
             driver.findElement(By.id("notifyBtnInMcNotifyCustomer")).click();
            Thread.sleep(4000);
            driver.findElement(By.id("custNotificationInMcNotifyCustWindow")).sendKeys(properties.getProperty("Compliant_Short_Description"));
            Thread.sleep(4000);
            driver.findElement(By.id("notifyBtnInMcNotifyCustWindow")).click();
            Thread.sleep(4000);
    ...
    private boolean selectRecdQAReviewNotifyCustomer(String MCNumber, boolean isRecordSelected, int count) throws InterruptedException {
...
//                    driver.findElement(By.cssSelector("#mcNotifyCustomerContainer > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();//next page in Document approve list
                   Helper.clickElement(driver, By.cssSelector("#mcNotifyCustomerContainer > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"));
                	Thread.sleep(3000);
                	Helper.waitLoadRecords(driver, By.cssSelector("#mcNotifyCustomerContainer > div > div.jtable-busy-message[style='display: none;']"));
