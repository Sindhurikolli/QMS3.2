...
public class MultiUsersAssignRoleDeviationTest{

	static File resultFile = new File("QMS User Details for Automation.xlsx");
    public static DataFormatter formatter = new DataFormatter();

    @Test(dataProvider = "readExcelFile")
    public void toMultiUserCreation(String DeviationUserE_Code, String DeviationRoleName ) throws Throwable {
        Thread.sleep(1000);


		Thread.sleep(5000);
        PropertiesConfiguration properties = new PropertiesConfiguration("src\\test\\java\\QMSUIProperties\\QMS.properties");
		properties.setProperty("DeviationUserE_Code", DeviationUserE_Code);
		properties.setProperty("DeviationRoleName", DeviationRoleName);
		properties.save();
		
        Thread.sleep(5000);
        QMSLoginDetails login = new QMSLoginDetails();
        login.setUp();
        System.out.println("Role Assigned for - "+DeviationUserE_Code);
        DeviationAssignRole role = new DeviationAssignRole();
        role.DeviationAssignRoleMethod();
    ...
    public static Object[][] readExcelFile() throws InvalidFormatException, IOException {
        ...
        XSSFSheet sh = wb.getSheet("DEVRole");
