package com.pss.qms.CAPARoleCreation;
...
public class InitiatorRoleCreation extends CAorPALoginDetails {
...
//		try {
			...
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "InitiatorRoleCreation"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("InitiatorRoleCreation", "PSS-QMS-054",
					"Pass");
        ...
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "create role menu",sno,false);
        Thread.sleep(5000);
        createRoleMethod();
        document.close();
		writer.close();
...
//    }
    ...
    private void createRoleMethod() throws Exception {
    	sno++;
        Thread.sleep(1000);
        driver.findElement(By.id("roleNameInCreateRoleInCapa")).sendKeys(properties.getProperty("Role_Name1"));
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter the role name",sno,false);
        Thread.sleep(3000);
        sno++;
        driver.findElement(By.id("descriptionInCreateRoleInCapa")).sendKeys(properties.getProperty("Description"));
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter the description",sno,false);
        Thread.sleep(3000);
        sno++;
