...
//		try {
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("Close Audit", "PSS-QMS-019", "Pass");
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					...
					if (AMId.equalsIgnoreCase(AMNumberSequence)) {
						driver.findElement(
								By.xpath("//*[@id=\"auditsListTableInCLoseAuditApprove\"]/div/table/tbody/tr/td[6]")).click();
						isRecordSelected = true;
						break;
					}
				}
