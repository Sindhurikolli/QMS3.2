...
public class CFTReviewRejectRA extends RALoginDetails {

	@Test
	public void CFTReviewRejectRA() throws Exception {

		document = new Document(PageSize.A4, 36, 36, 20, 20);
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CFTReviewRejectRA" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("CFTReviewRejectRA", "PSS-QMS-001", "Pass");
...
//         Thread.sleep(3000);
	...
	private void methodToDoCFTReviewRejectRA() throws Exception {

		int count = 0;
		boolean isRecordSelected = false;
		String RASequence = properties.getProperty("RA_NUMBER");
		isRecordSelected = selectRecdCFTReviewRejectRA(RASequence, isRecordSelected, count);
		Thread.sleep(1000);
		if (isRecordSelected) {
			sno++;
...
//			Thread.sleep(1000);
//			sno++;
			driver.findElement(By.id("rejectActivityInCheckInRaCftReview")).click();
	...
	private boolean selectRecdCFTReviewRejectRA(String RANumber, boolean isRecordSelected, int count)
