 ...
 * @author likhitha
 */
public class InvestigationRejectMC extends MCLoginDetails {

	@Test
	public void InvestigationRejectMC() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "InvestigationRejectMC"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("InvestigationRejectMC", "PSS-QMS-024", "Pass");
...
//        Thread.sleep(2000);
		...
		if (isRecordSelected) {
			...
			Thread.sleep(4000);
			sno++;
			driver.findElement(By.id("rejectedCommentsInInvActionsInInvPlan"))
					.sendKeys(properties.getProperty("MC_994"));
...
//            Thread.sleep(2000);
	...
	private boolean selectRecdInvestigationMC(String DeviationNumber, boolean isRecordSelected, int count)
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						if (DeviationNumber.equalsIgnoreCase(capaNumberSequence)) {
							driver.findElement(By.xpath(
									".//*[@id='investigationTableContainer']/div/table/tbody/tr[ " + i + " ]/td[6]"))
									.click();
					...
					String capaNumberSequence = driver
							...
							.getText();
					if (DeviationNumber.equalsIgnoreCase(capaNumberSequence)) {
						driver.findElement(By.xpath(".//*[@id='investigationTableContainer']/div/table/tbody/tr/td[6]"))
								.click();
						isRecordSelected = true;
						break;
					}
