...
public class QAReviewerCAPA extends AMLoginDetails {
 
	 @Test
	    public void methodtodoQAReviewerCAPA() throws InterruptedException, IOException, DocumentException, Exception {
//			try {
		  	...
		  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
				...
				writer.open();
				HeaderFooterPageEvent event = new HeaderFooterPageEvent("QAReviewerCAPA", "PSS-QMS-009",
						"Pass");
	     ...
	     im = Image.getInstance(input);
	         ...
	         sno++;
	         driver.findElement(By.xpath("//*[@id=\"capaReviewMenuId\"]/a")).click();
	         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on CAPA Review & Approval Menu",sno,false);
	        todoQAReviewerCAPA();
	        document.close();
			writer.close();
...
//		}
	    ...
	    private void todoQAReviewerCAPA() throws Exception {
	      
	    	 int count = 0;
	         boolean isRecordSelected = false;
	         String CAPANumber = properties.getProperty("CAPA_NUMBER_AM");
	         isRecordSelected = selectRecdQAReviewerCAPA(CAPANumber, isRecordSelected, count);
	         if (isRecordSelected) {
	        	 sno++;
	             ...
	             document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Next Button",sno,false); 
	             sno++;
	             Select QAFinal = new Select(driver.findElement(By.id("qaFinalWorkFlowUserInCapaReview")));
	             QAFinal.selectByIndex(3);
	             document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select The QAHOD",sno,false); 
	             sno++;
	             driver.findElement(By.xpath("//*[@id=\"capaReviewFormContentsDiv\"]/div[3]/ul/li[2]/a")).click();
	             document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Next Button",sno,false); 
	             sno++;
	             driver.findElement(By.id("commentsInCapaReviewForm")).sendKeys(properties.getProperty("Task_Desc_2000"));
		...
		private boolean selectRecdQAReviewerCAPA(String CAPANumber, boolean isRecordSelected, int count) throws InterruptedException {
