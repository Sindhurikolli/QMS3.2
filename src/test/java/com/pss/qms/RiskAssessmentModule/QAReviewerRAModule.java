...
public class QAReviewerRAModule extends RALoginDetails {

	@Test
	public void QAReviewerRA() throws Exception {

		document = new Document(PageSize.A4, 36, 36, 20, 20);
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "QAReviewerRAModule"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("QAReviewerRAModule", "PSS-QMS-006", "Pass");
...
//         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on MyActivities Tab",sno,false); 
	...
	private void methodToDoQAReviewRA() throws Exception {

		Thread.sleep(5000);
		JavascriptExecutor scl = ((JavascriptExecutor) driver);
		scl.executeScript("window.scrollBy(0,50)");
		Thread.sleep(3000);
		int count = 0;
		boolean isRecordSelected = false;
		String RASequence = properties.getProperty("RA_NUMBER");
		isRecordSelected = selectRecdQAReviewRA(RASequence, isRecordSelected, count);
		Thread.sleep(1000);
		if (isRecordSelected) {
			sno++;
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document,
					"Click On Customer Notification RadioButton", sno, false);
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("coutinueOtherOpinionInPaQaReview")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On CrossFunctional Team Review",
					sno, false);
			Thread.sleep(1000);
			sno++;
...
//          }
	...
	private boolean selectRecdQAReviewRA(String RANumber, boolean isRecordSelected, int count)
			throws InterruptedException {
