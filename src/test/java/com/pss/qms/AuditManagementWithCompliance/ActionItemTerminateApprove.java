...
public class ActionItemTerminateApprove extends AMLoginDetails {
    
   @Test
    public void ActionItemTerminationApprove() throws InterruptedException, IOException, DocumentException, Exception {
//		try {
			
			document = new Document(PageSize.A4, 36, 36, 20, 20);
		  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
	           document = new Document();
				output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ActionItemTerminateApprove"
				...
				writer.open();
				HeaderFooterPageEvent event = new HeaderFooterPageEvent("ActionItemTerminate Approve", "PSS-QMS-001",
						"Pass");
...
//        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on MyActivities Tab",sno,false);
        ...
        methodToDoActionItemTerminateApprove();
        Thread.sleep(3000);
        document.close();
		writer.close();
//	} catch (Exception e) {
...
//	}
    ...
    private void methodToDoActionItemTerminateApprove() throws Exception {
        int count = 0;
...
//        String sdf = new SimpleDateFormat("YY").format(date);
        ...
        isRecordSelected = selectRecdActionItemTerminateApprove(ActionItemNumber,isRecordSelected, count);
        Thread.sleep(3000);
        if (isRecordSelected) {
          sno++;
...
//            document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter The Comments "+"  PSS-QMS-SS-032",sno,false);
             driver.findElement(By.id("appBtnIdInAmActItmTerm")).click();
...
private boolean selectRecdActionItemTerminateApprove(String ActionItemNumber, boolean isRecordSelected, int count) throws InterruptedException {
        ...
        if (perPageNoOfRecordsPresent > 0) {
            while (noOfRecordsChecked < totalNoOfRecords) {
                if (totalNoOfRecords > 1) {
                    ...
                    String AIcapaNumberSequence = driver.findElement(By.xpath("//*[@id=\"actItmTermRecordInTerminateAppTable\"]/div/table/tbody/tr/td[4]")).getText();
                    if (ActionItemNumber.equalsIgnoreCase(AIcapaNumberSequence)) {
