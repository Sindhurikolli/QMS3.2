...
public class CloseComplaint extends MCLoginDetails {
	@Test

    public void CloseComplaint() throws InterruptedException {
    	try {

			document = new Document();
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CloseComplaint"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("CloseComplaint", "PSS-QMS-058",
					"Pass");
...
//    Thread.sleep(20000);
    ...
    wait.until(ExpectedConditions.invisibilityOf(driver.findElement(By.cssSelector("#mcCloseMCTable > div > div.jtable-busy-message"))));
    ...
    private void methodToDoCloseComplaintMC() throws Exception {
        ...
        if (isRecordSelected) {
            ...
            document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Established radio button", sno,false);
        sno++;
        Thread.sleep(8000);
        driver.findElement(By.xpath("//*[@id=\"supportingDocsJtableInMcCloser\"]/div/div[3]/div[2]/span/span[2]/b")).click();
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Add Supprting documents", sno,false);
        sno++;
        Thread.sleep(8000);
    ...
    private boolean selectRecdCloseComplaintMC(String MarketComplaintsId, boolean isRecordSelected, int count) throws InterruptedException {
        ...
        if (perPageNoOfRecordsPresent > 0) {
            String a = driver.findElement(By.xpath("//*[@id=\"mcCloseMCTable\"]/div/div[4]/div[2]/span")).getText();// For Ex: Showing 1-1 of 1
        ...
        if (perPageNoOfRecordsPresent > 0 && count == 0) {
            if ((totalNoOfRecords > 1) && ((MarketComplaintsId == null) || ("".equalsIgnoreCase(MarketComplaintsId)))) {
            	MarketComplaintsId = driver.findElement(By.xpath("//*[@id=\"mcCloseMCTable\"]/div/table/tbody/tr/td[19]")).getText();//documentType
            ...
            while (noOfRecordsChecked < totalNoOfRecords) {
                if (totalNoOfRecords > 1) {
                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
                        String capaNumberSequence = driver.findElement(By.xpath("//*[@id=\"mcCloseMCTable\"]/div/table/tbody/tr[ " + i + " ]/td[19]")).getText();//documentTypeName
                        if (MarketComplaintsId.equalsIgnoreCase(capaNumberSequence)) {
                            driver.findElement(By.xpath("//*[@id=\"mcCloseMCTable\"]/div/table/tbody/tr[ " + i + " ]/td[19]")).click();
                    ...
                    Thread.sleep(3000);
                    String capaNumberSequence = driver.findElement(By.xpath("//*[@id=\"mcCloseMCTable\"]/div/table/tbody/tr/td[19]")).getText();
                    Thread.sleep(3000);
                    if (MarketComplaintsId.equalsIgnoreCase(capaNumberSequence)) {
                        Thread.sleep(3000);
                ...
                if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
                    driver.findElement(By.cssSelector("#mcCloseMCTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();//next page in Document approve list
                    Thread.sleep(3000);
