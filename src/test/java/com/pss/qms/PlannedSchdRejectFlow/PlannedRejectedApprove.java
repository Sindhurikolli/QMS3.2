 ...
 * @author likhitha
 */
public class PlannedRejectedApprove extends AMLoginDetails {
    
	@Test
    public void PlannedRejectApprove() throws InterruptedException, IOException, DocumentException, Exception {
//		try {
			document = new Document(PageSize.A4, 36, 36, 20, 20);
		  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "PlannedRejectedApprove"
            ...
            writer.open();
            HeaderFooterPageEvent event = new HeaderFooterPageEvent("PlannedReject Approve", "PSS-QMS-001",
					"Pass");
...
//	}
    ...
    private void methodToDoPlanRejectApprove() throws Exception {
        ...
        if (isRecordSelected) {
            ...
            document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On Next Button",sno,false);
            sno++;
            driver.findElement(By.id("commentsInAmAuditPlanRej")).sendKeys(properties.getProperty("AUDIT_COMMENTS"));
    ...
    private boolean selectRecdPlanRejectApprove(String AuditId, boolean isRecordSelected, int count) throws InterruptedException {
        ...
        if (perPageNoOfRecordsPresent > 0) {
            String a = driver.findElement(By.xpath("//*[@id=\"rejectedAuditPlanTablesContainer\"]/div/div[4]/div[2]/span")).getText();// For Ex: Showing 1-1 of 1
        ...
        if (perPageNoOfRecordsPresent > 0 && count == 0) {
            if ((totalNoOfRecords > 1) && ((AuditId == null) || ("".equalsIgnoreCase(AuditId)))) {
            	AuditId = driver.findElement(By.xpath("//*[@id=\"rejectedAuditPlanTablesContainer\"]/div/table/tbody/tr[1]/td[3]")).getText();//documentType
            } else if ((AuditId == null) || ("".equalsIgnoreCase(AuditId))) {
            	AuditId = driver.findElement(By.xpath("//*[@id=\"rejectedAuditPlanTablesContainer\"]/div/table/tbody/tr/td[3]")).getText();//documentType
            ...
            while (noOfRecordsChecked < totalNoOfRecords) {
                if (totalNoOfRecords > 1) {
                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
                        String AuditNumberSequence = driver.findElement(By.xpath(".//*[@id='rejectedAuditPlanTablesContainer']/div/table/tbody/tr[ " + i + "]/td[3]")).getText();//documentTypeName
                    ...
                    String AuditNumberSequence = driver.findElement(By.xpath("//*[@id=\"rejectedAuditPlanTablesContainer\"]/div/table/tbody/tr/td[3]")).getText();
                    if (AuditId.equalsIgnoreCase(AuditNumberSequence)) {
                ...
                if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
                    driver.findElement(By.cssSelector("#rejectedAuditPlanTablesContainer > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();//next page in Document approve list
                    Thread.sleep(3000);
