	...
	private boolean selectRecdCAPerformTaskCAPA(String CAPANumber, boolean isRecordSelected, int count)
		...
		if (perPageNoOfRecordsPresent > 0) {
			...
			WebElement element10 = driver
					.findElement(By.xpath("//*[@id=\"prevActPerformTable\"]/div/div[4]/div[2]/span"));

			jse10.executeScript("arguments[0].scrollIntoView(true);", element10);

			String a = driver.findElement(By.xpath("//*[@id=\"prevActPerformTable\"]/div/div[4]/div[2]/span"))
					.getText();// For Ex: Showing 1-1 of 1
			String[] parts = a.split(" of ");
			try {
				totalNoOfRecords = Integer.parseInt(parts[1].trim());
				...
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						String capaNumberSequence = driver
								.findElement(By.xpath(
										"//*[@id=\"prevActPerformTable\"]/div/table/tbody/tr[ " + i + " ]/td[5]"))
								.getText();// documentTypeName
						if (CAPANumber.equalsIgnoreCase(capaNumberSequence)) {
							driver.findElement(
									By.xpath("//*[@id=\"prevActPerformTable\"]/div/table/tbody/tr[ " + i + " ]/td[5]"))
									.click();
