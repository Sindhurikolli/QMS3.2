...
public class ExternalScheduleReject extends AMLoginDetails {

	@Test
	public void ExternalSchdReject() throws Exception {

//		try {

			document = new Document(PageSize.A4, 36, 36, 20, 20);
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ExternalScheduleReject"
					+ (new Random().nextInt()) + ".pdf";
			fos = new FileOutputStream(output);
			writer = PdfWriter.getInstance(document, fos);
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("ExternalSchd Reject", "PSS-QMS-002", "Pass");
...
//	}
	...
	private void methodToDoExternalSchdReject() throws Exception {

		int count = 0;
		boolean isRecordSelected = false;
		String AuditId = properties.getProperty("EXTERNAL_AUDIT_NAME_WITH_AI_REJECT_FLOW");
		isRecordSelected = selectRecdExternalSchdReject(AuditId, isRecordSelected, count);
		Thread.sleep(2000);
		sno++;
	...
	private boolean selectRecdExternalSchdReject(String AuditId, boolean isRecordSelected, int count)
