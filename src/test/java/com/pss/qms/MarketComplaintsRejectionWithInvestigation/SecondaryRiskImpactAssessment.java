...
public class SecondaryRiskImpactAssessment extends MCLoginDetails {

	@Test
	public void SecondaryRiskImpactAssessment() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "SecondaryRiskImpactAssessment"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("SecondaryRiskImpactAssessment", "PSS-QMS-047", "Pass");
...
//	    Thread.sleep(20000);
	...
	private void methodToDoInvestigationMC() throws Exception {
		...
		if (isRecordSelected) {
			Thread.sleep(4000);
			sno++;
			driver.findElement(By.id("impactAssessmentDescription")).sendKeys(properties.getProperty("MC_300"));
				...
				if (totalNoOfRecords > 1) {
					...
					if (MCNumber.equalsIgnoreCase(capaNumberSequence)) {
						driver.findElement(By.xpath(
								"//*[@id=\"impactAssessmentInvTableContainer\"]/div/table/tbody/tr[1]/td[20]/button"))
								.click();
						isRecordSelected = true;
						break;
					}
