...
public class QMSCoOrdinatorRoleBaseAccess extends CAorPALoginDetails {
...
//		try {
			...
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "QMSCoOrdinatorRoleBaseAccess"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("QMSCoOrdinatorRoleBaseAccess", "PSS-QMS-061",
					"Pass");
...
//    }
    ...
    private void createRoleMethod() throws Exception {
        ...
        sno++;
         ...
         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Check on capa review menu",sno,false);
         Thread.sleep(3000); 
         sno++;
         driver.findElement(By.id("caReassignTaskMenu")).click();
         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Check on caReassign task menu",sno,false);
         Thread.sleep(3000); 
         sno++;
         driver.findElement(By.id("capaEffectiveChkOverDue")).click();
         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Check on capa effective  check overdue ",sno,false);
         Thread.sleep(3000); 
         sno++;
         driver.findElement(By.id("reqOverDueDateAppMenu")).click();
         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Check on req overdue date approval ",sno,false);
         Thread.sleep(3000); 
         sno++;
         driver.findElement(By.id("selectAllForCapaReports")).click();
         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Check on select All capa reports",sno,false);
         Thread.sleep(3000); 
         sno++;
         driver.findElement(By.id("selectAllForCapaAuditTrails")).click();
         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Check on SelectAll capa audittrails ",sno,false);
         Thread.sleep(3000);
