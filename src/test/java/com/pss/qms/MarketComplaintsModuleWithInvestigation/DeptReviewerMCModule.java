...
public class DeptReviewerMCModule extends MCLoginDetails {
    @Test
    public void DeptReviewerMC() throws InterruptedException {
        Thread.sleep(2000);
        driver.findElement(By.xpath(".//*[@id='marketComplaints_tile_Id']/div/div/div/h2")).click();
        Thread.sleep(10000);
        methodToDoDeptReviewMC();
    }

    private void  methodToDoDeptReviewMC() throws InterruptedException {
        int count = 0;
        ...
        String chgCtrlNoWithPlantCode = DepartmentCode + "/" + changeCtrlSequence;
        Date date = new Date();
        String sdf = new SimpleDateFormat("YY").format(date);
        String chgCtrlId = "/009";
        String MCNumber = chgCtrlNoWithPlantCode + sdf + chgCtrlId;
        isRecordSelected = selectRecdDeptReviewMC(MCNumber, isRecordSelected, count);
        Thread.sleep(1000);
        if(isRecordSelected) {
            driver.findElement(By.id("commentsInMcDeptReview")).sendKeys(properties.getProperty("Compliant_Short_Description"));
            Thread.sleep(1000);
            driver.findElement(By.id("submitBtnIdInMcDeptReview")).click();
            Thread.sleep(1000);
...
private boolean selectRecdDeptReviewMC(String MCNumber, boolean isRecordSelected, int count) throws InterruptedException {
  ...
  String capaNumberSequence = driver.findElement(By.xpath(".//*[@id='mcReviewTableContailner']/div/table/tbody/tr[ " + i + "]/td[3]")).getText();//documentTypeName
                    ...
                    if (MCNumber.equalsIgnoreCase(capaNumberSequence)) {
                        driver.findElement(By.xpath(".//*[@id='mcReviewTableContailner']/div/table/tbody/tr/td[45]/button")).click();
                        isRecordSelected = true;
                        break;
                    }
                }
