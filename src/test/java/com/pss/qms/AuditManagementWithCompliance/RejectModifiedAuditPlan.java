...
public class RejectModifiedAuditPlan extends AMLoginDetails
{
...
//		try {
			...
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "RejectModifiedAuditPlan"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("RejectModifiedAuditPlan", "PSS-QMS-001",
					"Pass");
...
//	}
    ...
    private void methodToDoRejectModifyAuditPlan() throws Exception {
        int count = 0;
        boolean isRecordSelected = false;
         String AMId = properties.getProperty("PLAN_NAME_PLANNED_SCHEDULE");
        isRecordSelected = selectRecdRejectModifyAuditPlan(AMId, isRecordSelected, count);
        if (isRecordSelected) {
        	
  
	        sno++;
            driver.findElement(By.id("commentsInModifyAmAuditPlanApp")).sendKeys(properties.getProperty("AUDIT_COMMENTS"));
            document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Comments",sno,false); 
            sno++;
            driver.findElement(By.id("rejectBtnInModifyAuditPlanApp")).click();
	...
	private boolean selectRecdRejectModifyAuditPlan(String AMId, boolean isRecordSelected, int count) throws InterruptedException {
