 ...
 * @author likhitha
 */
public class AuditeeResponseApprovalAdhocAM extends AMLoginDetails {
...
//			try {
				...
				output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "AuditeeResponseApprovalAdhocAM"
						+ (new Random().nextInt()) + ".pdf";
				fos = new FileOutputStream(output);

				writer = PdfWriter.getInstance(document, fos);
			    writer.open();
				HeaderFooterPageEvent event = new HeaderFooterPageEvent("AuditeeResponse Approve", "PSS-QMS-009",
						"Pass");
...
//	        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Approvals Menu",sno,false);
	        sno++;
	        driver.findElement(By.xpath("//*[@id=\"auditApprovalMenus\"]/div[6]/a")).click();
...
//		}
	    ...
	    private boolean selectRecdAuditeeResponseApprove(String AMId, boolean isRecordSelected, int count) throws InterruptedException {
	        ...
	        if (perPageNoOfRecordsPresent > 0 && count == 0) {
	            if ((totalNoOfRecords > 1) && ((AMId == null) || ("".equalsIgnoreCase(AMId)))) {
	            	AMId = driver.findElement(By.xpath("//*[@id=\"auditsContainerInReviewAuditFindingAppForm\"]/div/table/tbody/tr[1]/td[3]")).getText();//documentType
	            } else if ((AMId == null) || ("".equalsIgnoreCase(AMId))) {
	            	AMId = driver.findElement(By.xpath("//*[@id=\"auditsContainerInReviewAuditFindingAppForm\"]/div/table/tbody/tr/td[3]")).getText();//documentType
	            ...
	            while (noOfRecordsChecked < totalNoOfRecords) {
	                if (totalNoOfRecords > 1) {
	                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
	                        String AMNumberSequence = driver.findElement(By.xpath(".//*[@id='auditsContainerInReviewAuditFindingAppForm']/div/table/tbody/tr[ " + i + "]/td[3]")).getText();//documentTypeName
	                        if (AMId.equalsIgnoreCase(AMNumberSequence)) {
	                            driver.findElement(By.xpath("//*[@id=\"auditsContainerInReviewAuditFindingAppForm\"]/div/table/tbody/tr[ " + i + " ]/td[3]")).click();
	                    ...
	                    String AMNumberSequence = driver.findElement(By.xpath("//*[@id=\"auditsContainerInReviewAuditFindingAppForm\"]/div/table/tbody/tr/td[5]")).getText();
	                    if (AMId.equalsIgnoreCase(AMNumberSequence)) {
