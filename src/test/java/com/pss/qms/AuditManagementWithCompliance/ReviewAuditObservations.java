...
public class ReviewAuditObservations extends AMLoginDetails {
     ...
     @Test
    public void ReviewAuditObs() throws InterruptedException, IOException, DocumentException, Exception {
//		try {
			document = new Document(PageSize.A4, 36, 36, 20, 20);
		  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReviewAuditObservations"
					+ (new Random().nextInt()) + ".pdf";
			fos = new FileOutputStream(output);

			writer = PdfWriter.getInstance(document, fos);
            writer.open();
            HeaderFooterPageEvent event = new HeaderFooterPageEvent("ReviewAudit Obs", "PSS-QMS-001",
					"Pass");
...
//        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on MyActivities Tab",sno,false);
        sno++; 
        driver.findElement(By.xpath("//*[@id=\"internalAuditPerformMenus\"]/div[2]/a")).click();
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on AuditResponse Review Menu",sno,false);
        methodToDoReviewAuditObs();
        Thread.sleep(3000);
        document.close();
		writer.close();
...
//	}
    ...
    private void methodToDoReviewAuditObs() throws Exception {
        int count = 0;
        boolean isRecordSelected = false;
        String AMId = properties.getProperty("PLAN_NAME_PLANNED_SCHEDULE");
        isRecordSelected = selectRecdReviewAuditObs(AMId, isRecordSelected, count);
        Thread.sleep(3000);
        if (isRecordSelected) {
        	sno++;
            ...
            document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Next Button",sno,false); 
            sno++;
            driver.findElement(By.id("commentsInAmReviewAuditFindingForm")).sendKeys(properties.getProperty("AUDIT_COMMENTS_1500"));
    ...
    private boolean selectRecdReviewAuditObs(String AMId, boolean isRecordSelected, int count) throws InterruptedException {
