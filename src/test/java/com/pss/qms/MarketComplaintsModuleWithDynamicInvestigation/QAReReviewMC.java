...
public class QAReReviewMC extends MCLoginDetails {
	...
	public void QAReviewerMCModule() throws Exception {
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("QAReReviewMC", "PSS-QMS-008", "Pass");
...
//    Thread.sleep(20000);
	...
	private void methodToDoQAReReviewerMC() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(4000);
			driver.findElement(By.id("locTreeInQmsProdReg_2_ico")).click();
...
//           WebElement element = driver.findElement(By.id("cftChildReqInMcReview"));
...
//           To Select CFT User
			...
			String cftFirstName = properties.getProperty("CFT_Department");
...
//             driver.findElement(By.id("addBtnInActionItemAddDlg")).click();
	...
	private boolean selectingTheCFTReview(String cftFirstName, boolean isRecordSelectedForCFT, int count2) throws InterruptedException {
		...
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
         ...
         int noOfRecordsChecked = 0;
//         if (perPageNoOfRecordsPresent > 0) {
//             String a = driver.findElement(By.xpath("//*[@id=\"cftDeptTableWindowContainer\"]/div/div[4]/div[2]/span")).getText();// For Ex: Showing 1-1 of 1
//             String[] parts = a.split(" of ");
//             try {
//                 totalNoOfRecords = Integer.parseInt(parts[1].trim());
//             } catch (Exception e) {
...
//         }
		...
		if (perPageNoOfRecordsPresent > 0 && count2 == 0) {
			if (((cftFirstName == null) || ("".equalsIgnoreCase(cftFirstName)))) {
				cftFirstName = driver
						.findElement(By.xpath("//*[@id=\"cftDeptTableWindowContainer\"]/div/table/tbody/tr[1]/td[3]"))
						.getText();// documentType
			} else if ((cftFirstName == null) || ("".equalsIgnoreCase(cftFirstName))) {
				cftFirstName = driver
						.findElement(By.xpath("//*[@id=\"cftDeptTableWindowContainer\"]/div/table/tbody/tr/td[3]"))
						.getText();// documentType
			...
			if (perPageNoOfRecordsPresent > 1) {
				for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
					String cftReviewerFullName = driver
							.findElement(By.xpath(
									"//*[@id=\"cftDeptTableWindowContainer\"]/div/table/tbody/tr[" + i + "]/td[3]"))
							.getText();// documentTypeName
					System.out.println("cftReviewerFullName: " + cftReviewerFullName);
					if (cftFirstName.equalsIgnoreCase(cftReviewerFullName)) {
						driver.findElement(By
								.xpath("//*[@id=\"cftDeptTableWindowContainer\"]/div/table/tbody/tr[" + i + "]/td[3]"))
								.click();
				...
				String cftReviewerFullName = driver
						...
						.getText();
				if (cftFirstName.equalsIgnoreCase(cftReviewerFullName)) {
					driver.findElement(By.xpath("//*[@id=\"cftDeptTableWindowContainer\"]/div/table/tbody/tr[1]/td[3]"))
							.click();
					isRecordSelectedForCFT = true;
                 ...
                 noOfRecordsChecked += perPageNoOfRecordsPresent;
                 if ((!isRecordSelectedForCFT) && (noOfRecordsChecked < totalNoOfRecords)) {
                     driver.findElement(By.cssSelector("#cftDeptTableWindowContainer > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();//next page in Document approve list
                     Thread.sleep(3000);
                     table = driver.findElement(By.id("cftDeptTableWindowContainer"));//Document Tree approve table
                     tableBody = table.findElement(By.tagName("tbody"));
                     perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
                 }
			 }
