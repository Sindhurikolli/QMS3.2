package com.pss.qms.AuditManagementtobedeleted;
...
public class ActionItemApprove2 extends AMLoginDetails {
	   ...
	   @Test
		    public void ActionItemApprove() throws InterruptedException, IOException, DocumentException, Exception {
//				try {
					document = new Document(PageSize.A4, 36, 36, 20, 20);
				  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
					  document = new Document();
						output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ActionItemApproveAdhocAM"
						...
						writer.open();
						HeaderFooterPageEvent event = new HeaderFooterPageEvent("ActionItem Approve", "PSS-QMS-011",
								"Pass");
				 ...
				 im = Image.getInstance(input);
					...
					document.add(new Paragraph(sno+"."+"Enter the username, password, Select Audit Management and click on login button"+ Utilities.prepareSSNumber(sno), font));
					document.add(im);
...
//		        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Action Item Menu",sno,false);
		        sno++;
		        driver.findElement(By.xpath("//*[@id=\"amActionItemsMenus\"]/div[3]/a")).click();
		        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Action Item Approve Menu",sno,false);
...
//		      	}
		    ...
		    private void methodToDoActionItem() throws Exception {
		        ...
		        String ActionId = properties.getProperty("AI_NUMBER_2");
		        String ActionItemNumber = ActionId;
		        isRecordSelected = selectRecdActionItem(ActionItemNumber,isRecordSelected, count);
		        Thread.sleep(5000);
		        if (isRecordSelected) {
		        	sno++;
		            driver.findElement(By.id("commentsInApproveForm")).sendKeys(properties.getProperty("AUDIT_COMMENTS"));
		    ...
		    private boolean selectRecdActionItem(String ActionItemNumber, boolean isRecordSelected, int count) throws InterruptedException {
		        ...
		        if (perPageNoOfRecordsPresent > 0) {
		            String a = driver.findElement(By.xpath("//*[@id=\"actItmApproveTable\"]/div/div[4]/div[2]/span")).getText();// For Ex: Showing 1-1 of 1
		        ...
		        if (perPageNoOfRecordsPresent > 0 && count == 0) {
		            if ((totalNoOfRecords > 1) && ((ActionItemNumber == null) || ("".equalsIgnoreCase(ActionItemNumber)))) {
		            	ActionItemNumber = driver.findElement(By.xpath("//*[@id=\"actItmApproveTable\"]/div/table/tbody/tr[1]/td[3]")).getText();//documentType
		            } else if ((ActionItemNumber == null) || ("".equalsIgnoreCase(ActionItemNumber))) {
		            	ActionItemNumber = driver.findElement(By.xpath("//*[@id=\"actItmApproveTable\"]/div/table/tbody/tr/td[3]")).getText();//documentType
		            ...
		            while (noOfRecordsChecked < totalNoOfRecords) {
		                if (totalNoOfRecords > 1) {
		                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
		                        String ActionitemNumberSequence = driver.findElement(By.xpath("//*[@id=\"actItmApproveTable\"]/div/table/tbody/tr[ " + i + " ]/td[3]")).getText();//documentTypeName
		                        if (ActionItemNumber.equalsIgnoreCase(ActionitemNumberSequence)) {
		                    ...
		                    String ActionitemNumberSequence = driver.findElement(By.xpath("//*[@id=\"actItmApproveTable\"]/div/table/tbody/tr/td[3]")).getText();
		                    if (ActionItemNumber.equalsIgnoreCase(ActionitemNumberSequence)) {
		                ...
		                if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
		                    driver.findElement(By.cssSelector("#actItmApproveTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();//next page in Document approve list
		                    Thread.sleep(3000);
