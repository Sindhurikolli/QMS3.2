...
//			try {
     	...
     	writer = PdfWriter.getInstance(document, fos);
					...
					writer.open();
					HeaderFooterPageEvent event = new HeaderFooterPageEvent("ActionItemReject Approve", "PSS-QMS-010",
							"Pass");
...
//	        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on MyActivities Tab",sno,false);
	        ...
	        methodToDoActionItemRejectApprove();
	        Thread.sleep(6000);
	        document.close();
			writer.close();
//		} catch (Exception e) {
...
//	    }
	    ...
	    private void methodToDoActionItemRejectApprove() throws Exception {
	        int count = 0;
	        boolean isRecordSelected = false;
	        String ActionId = properties.getProperty("AI_NUMBER");
	        String ActionItemNumber = ActionId;
	        isRecordSelected = selectRecdActionItemRejectApprove(ActionItemNumber,isRecordSelected, count);
	        Thread.sleep(3000);
	        if (isRecordSelected) {
	        	sno++;
...
//	            document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter The Comments "+"  PSS-QMS-SS-032",sno,false);
		...
		private boolean selectRecdActionItemRejectApprove(String ActionItemNumber, boolean isRecordSelected, int count) throws InterruptedException {
	        ...
	        if (perPageNoOfRecordsPresent > 0) {
	            while (noOfRecordsChecked < totalNoOfRecords) {
	                if (totalNoOfRecords > 1) {
	                    ...
	                    String AMNumberSequence = driver.findElement(By.xpath("//*[@id=\"actItmRejectTable\"]/div/table/tbody/tr/td[3]")).getText();
	                    if (ActionItemNumber.equalsIgnoreCase(AMNumberSequence)) {
