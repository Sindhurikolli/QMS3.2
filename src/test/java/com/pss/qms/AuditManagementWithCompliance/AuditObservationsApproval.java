 ...
 * @author likhitha
 */
public class AuditObservationsApproval extends AMLoginDetails {
    
   
     @Test
    public void AuditObservationApprove() throws InterruptedException, IOException, DocumentException, Exception {
//		try {
			document = new Document(PageSize.A4, 36, 36, 20, 20);
		  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "AuditObservationsApproval"
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("AuditObs Approve", "PSS-QMS-001",
					"Pass");
...
//        sno++;
//        driver.findElement(By.xpath("//*[@id=\"amMyActApprovalMenuId\"]/a/b")).click();
//        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Approvals Menu",sno,false);
        sno++;
        driver.findElement(By.xpath("//*[@id=\"auditApprovalMenus\"]/div[7]/a")).click();
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Audit Observations Menu",sno,false);
        methodToDoAuditObservationApprove();
        Thread.sleep(3000);
        document.close();
		writer.close();
...
//	}
    ...
    private void methodToDoAuditObservationApprove() throws Exception {
        int count = 0;
        boolean isRecordSelected = false;
        String AMId = properties.getProperty("PLAN_NAME_PLANNED_SCHEDULE");
        isRecordSelected = selectRecdAuditObservationApprove(AMId, isRecordSelected, count);
        Thread.sleep(3000);
        if (isRecordSelected) {
        	sno++;
    ...
    private boolean selectRecdAuditObservationApprove(String AMId, boolean isRecordSelected, int count) throws InterruptedException {
            ...
            while (noOfRecordsChecked < totalNoOfRecords) {
                if (totalNoOfRecords > 1) {
                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
                        String AINumberSequence = driver.findElement(By.xpath(".//*[@id='auditsContainerInEnterAuditFindingsForm']/div/table/tbody/tr[ " + i + "]/td[3]")).getText();//documentTypeName
                        if (AMId.equalsIgnoreCase(AINumberSequence)) {
                    ...
                    String AINumberSequence = driver.findElement(By.xpath("//*[@id=\"auditsContainerInEnterAuditFindingsForm\"]/div/table/tbody/tr/td[3]")).getText();
                    if (AMId.equalsIgnoreCase(AINumberSequence)) {
