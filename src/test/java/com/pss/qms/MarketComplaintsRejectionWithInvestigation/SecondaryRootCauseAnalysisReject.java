...
public class SecondaryRootCauseAnalysisReject extends MCLoginDetails {

	@Test
	public void RootCauseAnalysisReject() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "SecondaryRootCauseAnalysisReject"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("SecondaryRootCauseAnalysisReject", "PSS-QMS-042",
				"Pass");
...
//    Thread.sleep(20000);
	...
	private void methodToDoInvestigationMC() throws Exception {
		...
		if (isRecordSelected) {
			sno++;
			driver.findElement(By.id("rejectActivityInCheckInRootCA")).click();
...
//            document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Add new record", sno,false);
			Thread.sleep(4000);
			sno++;
			driver.findElement(By.id("rejectedCommentsInInvActionsInRootCA"))
...
//            Thread.sleep(2000);
			...
			WebElement element = driver.findElement(By.id("submitBtnInRejectRootCa"));
			jse.executeScript("arguments[0].scrollIntoView(true);", element);
			Thread.sleep(4000);
			driver.findElement(By.id("submitBtnInRejectRootCa")).click();
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					...
					String capaNumberSequence = driver
							.findElement(By
									.xpath("//*[@id=\"rootCauseAnalysisTableContainer\"]/div/table/tbody/tr[1]/td[5]"))
							.getText();
					if (MCNumber.equalsIgnoreCase(capaNumberSequence)) {
						driver.findElement(
								By.xpath("//*[@id=\"rootCauseAnalysisTableContainer\"]/div/table/tbody/tr[1]/td[5]"))
