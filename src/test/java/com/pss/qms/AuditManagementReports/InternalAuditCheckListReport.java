 ...
 * @author likhitha
 */
public class InternalAuditCheckListReport extends AMLoginDetails {
    
 @Test
    public void InternalAuditCheckListReport() throws InterruptedException, IOException, DocumentException, Exception {
//		try {
			document = new Document(PageSize.A4, 36, 36, 20, 20);
		  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			document = new Document();
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "InternalAuditCheckListReport"
            ...
            writer.open();
            HeaderFooterPageEvent event = new HeaderFooterPageEvent("InternalAuditCheckList Report", "PSS-QMS-001",
					"Pass");
        ...
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on CheckListReport Menu",sno,false);
        sno++;
...
//        sno++;
        driver.findElement(By.id("searchBtnInAmQmsOnChkListAuditReport")).click();
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On Search Button",sno,false);
        methodToDoInternalAuditCheckListReport();
        document.close();
		writer.close();
...
//	}
    ...
    private void methodToDoInternalAuditCheckListReport() throws Exception {
        int count = 0;
        boolean isRecordSelected = false;
        String AuditNumber = properties.getProperty("AUDIT_NUMBER_IN_REPORT");
        isRecordSelected = selectRecdInternalAuditCheckListReport(AuditNumber,isRecordSelected, count);
        Thread.sleep(10000);
        if (isRecordSelected) {
...
//            Thread.sleep(3000);
    ...
    private boolean selectRecdInternalAuditCheckListReport(String AuditNumber, boolean isRecordSelected, int count) throws InterruptedException {
        ...
        if (perPageNoOfRecordsPresent > 0 && count == 0) {
            if ((totalNoOfRecords > 1) && ((AuditNumber == null) || ("".equalsIgnoreCase(AuditNumber)))) {
            	AuditNumber = driver.findElement(By.xpath("//*[@id=\"auditsListTableContainer\"]/div/table/tbody/tr[1]/td[3]")).getText();//documentType
            } else if ((AuditNumber == null) || ("".equalsIgnoreCase(AuditNumber))) {
            	AuditNumber = driver.findElement(By.xpath("//*[@id=\"auditsListTableContainer\"]/div/table/tbody/tr/td[3]")).getText();//documentType
            ...
            while (noOfRecordsChecked < totalNoOfRecords) {
                if (totalNoOfRecords > 1) {
                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
                        String AuditNumberSequence = driver.findElement(By.xpath("//*[@id=\"auditsListTableContainer\"]/div/table/tbody/tr[ " + i + " ]/td[5]")).getText();//documentTypeName
                        if (AuditNumber.equalsIgnoreCase(AuditNumberSequence)) {
                            driver.findElement(By.xpath("//*[@id=\"auditsListTableContainer\"]/div/table/tbody/tr[ " + i + " ]/td[10]/button")).click();
                    ...
                    String AuditNumberSequence = driver.findElement(By.xpath("//*[@id=\"auditsListTableContainer\"]/div/table/tbody/tr/td[5]")).getText();
                    if (AuditNumber.equalsIgnoreCase(AuditNumberSequence)) {
                        driver.findElement(By.xpath("//*[@id=\"auditsListTableContainer\"]/div/table/tbody/tr/td[10]/button")).click();
                        isRecordSelected = true;
                        break;
                    }
                }
