...
public class QAApproverRejectRA extends RALoginDetails {

	@Test
	public void QAApproveRejectRA() throws Exception {

		document = new Document(PageSize.A4, 36, 36, 20, 20);
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "QAApproverRejectRA"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("QAApproverRejectRA", "PSS-QMS-001", "Pass");
...
//         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on MyActivities Tab",sno,false);  
	...
	private void methodToDoQAApproveRejectRA() throws Exception {
		...
		isRecordSelected = selectRecdQAApproveReject(RASequence, isRecordSelected, count);
		Thread.sleep(4000);
		if (isRecordSelected) {
			sno++;
	...
	private boolean selectRecdQAApproveReject(String RANumber, boolean isRecordSelected, int count)
