 ...
 * @author likhitha
 */
public class PlannedScheduleApprove extends AMLoginDetails {
    
	@Test
    public void PlanSchdApprove() throws InterruptedException, IOException, DocumentException, Exception {
//		try {
			document = new Document(PageSize.A4, 36, 36, 20, 20);
		  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "PlannedScheduleApprove"
            ...
            writer.open();
            HeaderFooterPageEvent event = new HeaderFooterPageEvent("PlanSchedule Approve", "PSS-QMS-001",
					"Pass");
...
//	}
    ...
    private void methodToDoPlanSchdApprove() throws Exception {
        int count = 0;
        boolean isRecordSelected = false;
        String AuditId = properties.getProperty("PLAN_NAME_PLANNED_SCHEDULE");
        isRecordSelected = selectRecdPlanSchdApprove(AuditId, isRecordSelected, count);
        if (isRecordSelected) {
        	sno++;
    ...
    private boolean selectRecdPlanSchdApprove(String AuditId, boolean isRecordSelected, int count) throws InterruptedException {
