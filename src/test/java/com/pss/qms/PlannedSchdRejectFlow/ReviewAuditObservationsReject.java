 ...
 * @author likhitha
 */
public class ReviewAuditObservationsReject extends AMLoginDetails {
    
	@Test
    public void ReviewAuditObsReject() throws InterruptedException, IOException, DocumentException, Exception {
//		try {
			document = new Document(PageSize.A4, 36, 36, 20, 20);
		  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReviewAuditObsReject"
            ...
            writer.open();
            HeaderFooterPageEvent event = new HeaderFooterPageEvent("ReviewAudit Obs Reject", "PSS-QMS-001",
					"Pass");
     ...
     driver.findElement(By.xpath("//*[@id=\"loginform\"]/div[7]/button[1]")).click();
      ...
      sno++;
      driver.findElement(By.xpath("//*[@id=\"reviewAuditFindingsMenuTag\"]/a")).click();
      document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Review Audit Observations Menu",sno,false); 
      sno++;
     methodToDoReviewAuditObsReject();
     document.close();
		writer.close();
...
//	}
    ...
    private void methodToDoReviewAuditObsReject() throws Exception {
        int count = 0;
        boolean isRecordSelected = false;
        String AuditId = properties.getProperty("PLAN_NAME_PLANNED_SCHEDULE");
        isRecordSelected = selectRecdReviewAuditObsReject(AuditId, isRecordSelected, count);
        if (isRecordSelected) {
            ...
            document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On Next Button",sno,false);
        	sno++;
            driver.findElement(By.id("commentsInAmReviewAuditFindingForm")).sendKeys(properties.getProperty("AUDIT_COMMENTS"));
    ...
    private boolean selectRecdReviewAuditObsReject(String AuditId, boolean isRecordSelected, int count) throws InterruptedException {
        ...
        if (perPageNoOfRecordsPresent > 0 && count == 0) {
            if ((totalNoOfRecords > 1) && ((AuditId == null) || ("".equalsIgnoreCase(AuditId)))) {
            	AuditId = driver.findElement(By.xpath("//*[@id=\"auditsContainerInReviewAuditFindingForm\"]/div/table/tbody/tr[1]/td[3]")).getText();//documentType
            } else if ((AuditId == null) || ("".equalsIgnoreCase(AuditId))) {
            	AuditId = driver.findElement(By.xpath("//*[@id=\"auditsContainerInReviewAuditFindingForm\"]/div/table/tbody/tr/td[3]")).getText();//documentType
            ...
            while (noOfRecordsChecked < totalNoOfRecords) {
                if (totalNoOfRecords > 1) {
                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
                        String AuditNumberSequence = driver.findElement(By.xpath(".//*[@id='auditsContainerInReviewAuditFindingForm']/div/table/tbody/tr[ " + i + "]/td[3]")).getText();//documentTypeName
                    ...
                    String AuditNumberSequence = driver.findElement(By.xpath("//*[@id=\"auditsContainerInReviewAuditFindingForm\"]/div/table/tbody/tr/td[3]")).getText();
                    if (AuditId.equalsIgnoreCase(AuditNumberSequence)) {
