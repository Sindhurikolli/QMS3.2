		...
		private boolean selectRecdActionItemReAssign(String ActionItemNumber, boolean isRecordSelected, int count) throws InterruptedException {
	        ...
	        if (perPageNoOfRecordsPresent > 0) {
	            while (noOfRecordsChecked < totalNoOfRecords) {
	                if (totalNoOfRecords > 1) {
	                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
	                        String AINumberSequence = driver.findElement(By.xpath("//*[@id=\"actItmReassignTable\"]/div/table/tbody/tr[ " + i + " ]/td[3]")).getText();//documentTypeName
	                    ...
	                    String AINumberSequence = driver.findElement(By.xpath("//*[@id=\"actItmReassignTable\"]/div/table/tbody/tr/td[3]")).getText();
	                    if (ActionItemNumber.equalsIgnoreCase(AINumberSequence)) {
