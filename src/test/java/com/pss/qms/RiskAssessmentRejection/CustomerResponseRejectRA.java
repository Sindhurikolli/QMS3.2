...
public class CustomerResponseRejectRA extends RALoginDetails {

	@Test
	public void CustomerResponseRejectRAMethod() throws Exception {

		document = new Document(PageSize.A4, 36, 36, 20, 20);
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CustomerResponseRejectRA"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("CustomerResponseRejectRA", "PSS-QMS-001", "Pass");
...
//         Thread.sleep(3000);
	...
	private void methodToDoQAReviewCustomerResponse() throws Exception {
		...
		isRecordSelected = selectRecdQAReviewAfterNotifyCustomer(RASequence, isRecordSelected, count);
		Thread.sleep(4000);
		if (isRecordSelected) {
			sno++;
			driver.findElement(By.id("custRespBtnInRaNotifyCustomer")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On Customer Response Button", sno,
					false);
			Thread.sleep(4000);
			/*
			 * JavascriptExecutor jse = (JavascriptExecutor) driver; WebElement element =
			 * driver.findElement(By.id("submitBtnInRaNotifyCustWindow"));
			 * jse.executeScript("arguments[0].scrollIntoView(true);", element);
			 */
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("needMoreInfoRadioInRaNotifyCustWindow")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On Revert Button", sno, false);
			sno++;
			driver.findElement(By.id("responseInRaNotifyCustWindow")).sendKeys(properties.getProperty("RA_2000"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Customer Response", sno, false);
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("custRevDocInRaNotifyCustWindow")).sendKeys(properties.getProperty("Document-2"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Upload The Document", sno, false);
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("submitBtnInRaNotifyCustWindow")).click();
	...
	private boolean selectRecdQAReviewAfterNotifyCustomer(String RANumber, boolean isRecordSelected, int count)
