...
public class CFTReviewMCModule extends MCLoginDetails {
	...
	public void CFTReviewMCModule1() throws Exception {
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("CFTReviewMCModule", "PSS-QMS-014", "Pass");
...
//				false);
	...
	private void methodToDoCFTReviewerMC() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("uploadSuppDocCFT_1")).sendKeys(properties.getProperty("Document-3"));
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						if (MCNumber.equalsIgnoreCase(capaNumberSequence)) {
							driver.findElement(By.xpath(
									"//*[@id=\"mcCFTReviewTable\"]/div/table/tbody/tr[ " + i + " ]/td[30]/button"))
									.click();
					...
					if (MCNumber.equalsIgnoreCase(capaNumberSequence)) {
						Thread.sleep(3000);
						driver.findElement(By.xpath("//*[@id=\"mcCFTReviewTable\"]/div/table/tbody/tr/td[30]/button"))
								.click();
						Thread.sleep(3000);
						isRecordSelected = true;
						break;
