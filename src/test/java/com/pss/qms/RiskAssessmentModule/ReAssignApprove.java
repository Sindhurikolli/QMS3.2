...
public class ReAssignApprove extends RALoginDetails {
	 
	  @Test
	    public void ApproveReAssignRA() throws InterruptedException, IOException, DocumentException, Exception {
//			try {

				document = new Document(PageSize.A4, 36, 36, 20, 20);
		  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
				output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReAssignApprove"
				...
				writer.open();
				HeaderFooterPageEvent event = new HeaderFooterPageEvent("ReAssignApprove", "PSS-QMS-001",
						"Pass");
				writer.setPageEvent(event);
				document.open();
	      driver.findElement(By.name("loginUserName")).sendKeys(properties.getProperty("REASSIGN-APP"));
...
//		}
	    ...
	    private void methodToDoReAssignApprove() throws Exception {
	        int count = 0;
	        boolean isRecordSelected = false;
	        String RASequence = properties.getProperty("RA_NUMBER");
	        isRecordSelected = selectRecdReAssignApprove(RASequence, isRecordSelected, count);
	        Thread.sleep(1000);
	        if (isRecordSelected) {
	           	sno++;
	            driver.findElement(By.id("commentsInFinalQaCcReview")).sendKeys(properties.getProperty("RA_SHORT_DESCRIPTION"));
	            document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Comments",sno,false);
	            Thread.sleep(1000);
	            sno++;
	            driver.findElement(By.id("submitBtnIdInRaQaApprover")).click();
	    ...
	    private boolean selectRecdReAssignApprove(String RANumber, boolean isRecordSelected, int count) throws InterruptedException {
	        ...
	        if (perPageNoOfRecordsPresent > 0) {
	            while (noOfRecordsChecked < totalNoOfRecords) {
	                if (totalNoOfRecords > 1) {
	                    ...
	                    String RANumberSequence = driver.findElement(By.xpath("//*[@id=\"riskAnalysisReviewTableContainer\"]/div/table/tbody/tr/td[4]")).getText();
	                    if (RANumber.equalsIgnoreCase(RANumberSequence)) {
