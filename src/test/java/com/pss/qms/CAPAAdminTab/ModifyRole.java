...
public class ModifyRole extends CAorPALoginDetails {
...
//		try {
			...
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("ModifyRole", "PSS-QMS-047",
					"Pass");
        ...
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Modify role menu",sno,false);
        Thread.sleep(8000);
        toModifyRole();
        document.close();
		writer.close();
...
//	}
    ...
    private void toModifyRole() throws Exception {
        Thread.sleep(4000);
        sno++;
        Select ExistingRoles = new Select(driver.findElement(By.id("roleInModifyRoleInCapa")));
        ExistingRoles.selectByIndex(7);
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select role from drop down list",sno,false);
        Thread.sleep(3000);
        sno++;
        driver.findElement(By.id("roleNameInModifyRoleInCapa")).clear();
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Clear the role name",sno,false);
        Thread.sleep(3000);
        sno++;
        driver.findElement(By.id("roleNameInModifyRoleInCapa")).sendKeys(properties.getProperty("MODIFY_ROLE_NAME"));
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter the modify role name",sno,false);
        Thread.sleep(3000);
        sno++;
        driver.findElement(By.id("RoleDescInModifyRoleInCapa")).clear();
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Clear the role description",sno,false);
        Thread.sleep(3000);
        sno++;
        driver.findElement(By.id("RoleDescInModifyRoleInCapa")).sendKeys(properties.getProperty("MODIFY_ROLE_NAME"));
        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter the role description",sno,false);
        Thread.sleep(3000);
        sno++;
        driver.findElement(By.id("subBtnIdInModifyRoleInCapa")).click();
