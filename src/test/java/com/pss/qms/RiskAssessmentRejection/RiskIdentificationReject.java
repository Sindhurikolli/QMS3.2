...
public class RiskIdentificationReject extends RALoginDetails {

	@Test
	public void RiskIdentificationReject() throws Exception {

		document = new Document(PageSize.A4, 36, 36, 20, 20);
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "RiskIdentificationReject"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("RiskIdentificationReject", "PSS-QMS-002", "Pass");
...
//         WebElement element = driver.findElement(By.xpath("//*[@id=\"approvalsGroupMenuId\"]/div/a"));
...
//         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on MyActivities Tab",sno,false);  
		...
		WebElement element1 = driver
				.findElement(By.xpath("//*[@id=\"riskAnalysisAppTableContainer\"]/div/div[4]/div[1]"));
		jse1.executeScript("arguments[0].scrollIntoView(true);", element1);
		Thread.sleep(1000);
		methodToDoRiskIdentificationReject();
		Thread.sleep(3000);
		document.close();
		writer.close();
	...
	private void methodToDoRiskIdentificationReject() throws Exception {

		Thread.sleep(4000);
		int count = 0;
		boolean isRecordSelected = false;
		String RASequence = properties.getProperty("RA_NUMBER");
		isRecordSelected = selectRecdRiskIdentificationReject(RASequence, isRecordSelected, count);
		Thread.sleep(4000);
		if (isRecordSelected) {
			sno++;
//        	JavascriptExecutor jse = (JavascriptExecutor) driver;
//        	WebElement element = driver.findElement(By.id("commentsInRaAppWin"));
//        	jse.executeScript("arguments[0].scrollIntoView(true);", element);
			driver.findElement(By.id("commentsInRaAppWin")).sendKeys(properties.getProperty("RA_2000"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Comments", sno, false);
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("rejectBtnInRaAppWin")).click();
...
//			}
	...
	private boolean selectRecdRiskIdentificationReject(String RANumber, boolean isRecordSelected, int count)
...
//					driver.findElement(By.cssSelector("#riskAnalysisAppTableContainer > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();// next page in Document approve list
					Helper.clickElement(driver, By.cssSelector("#riskAnalysisAppTableContainer > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"));
					Thread.sleep(3000);
					Helper.waitLoadRecords(driver, By.cssSelector("#riskAnalysisAppTableContainer > div > div.jtable-busy-message[style='display: none;']"));
