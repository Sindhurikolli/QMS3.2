...
public class RegisterReviewReject extends MCLoginDetails {

	@Test
	public void RegisterReviewReject() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "RegisterReviewReject"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("RegisterReviewReject", "PSS-QMS-002", "Pass");
...
//		String sdf = new SimpleDateFormat("YY").format(date);
		...
		if (isRecordSelected) {
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on CFT checkboox", sno, false);
			Thread.sleep(2000);
			sno++;
...
//			Thread.sleep(4000);
			...
			String InvFirstName = properties.getProperty("CFT_User_E-Code");
...
//                }
	...
	private boolean selectingTheInvReview(String InvFullName, boolean recordSelectedForInv, int count4)
		...
		int noOfRecordsChecked = 0;
//		if (perPageNoOfRecordsPresent > 0) {
//			String a = driver.findElement(By.xpath("//*[@id=\"cftTeamTableWindowContainer\"]/div/div[4]/div[2]/span"))
//					.getText();// For Ex: Showing 1-1 of 1
//			String[] parts = a.split(" of ");
//			try {
//				totalNoOfRecords = Integer.parseInt(parts[1].trim());
...
//						System.out.println("InvReviewFullName: " + InvFirstName);
					...
					if (InvFullName.equalsIgnoreCase(InvReviewFullName)) {
						driver.findElement(
								By.xpath("//*[@id=\"cftTeamTableWindowContainer\"]/div/table/tbody/tr/td[4]")).click();
						recordSelectedForInv = true;
