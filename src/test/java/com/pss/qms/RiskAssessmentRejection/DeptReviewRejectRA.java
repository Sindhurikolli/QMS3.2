...
public class DeptReviewRejectRA extends RALoginDetails {
	...
	public void DeptReviewerRA() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "DeptReviewRejectRA"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("DeptReviewRejectRA", "PSS-QMS-007", "Pass");
...
//         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on MyActivities Tab",sno,false);  
	...
	private void methodToDoDeptReviewRA() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("rejectActivityInCheckInRaDeptRev")).click();
