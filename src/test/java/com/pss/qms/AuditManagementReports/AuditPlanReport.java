...
public class AuditPlanReport extends AMLoginDetails {
    
   @Test
    public void AuditPlanReport() throws InterruptedException, IOException, DocumentException, Exception {
//		try {
			document = new Document(PageSize.A4, 36, 36, 20, 20);
		  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			document = new Document();
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "AuditPlanReport"
            ...
            writer.open();
            HeaderFooterPageEvent event = new HeaderFooterPageEvent("AuditPlanReport", "PSS-QMS-001",
					"Pass");
        ...
        sno++;
         ...
         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on AuditPlan Report Menu",sno,false);
...
//	}
    ...
    private void methodToDoAuditPlanReport() throws Exception {
        int count = 0;
        boolean isRecordSelected = false;
        String AuditNumber = properties.getProperty("AUDIT_NUMBER_IN_REPORT");
        isRecordSelected = selectRecdAuditPlanReport(AuditNumber,isRecordSelected, count);
        Thread.sleep(10000);
        if (isRecordSelected) {
...
//            Thread.sleep(3000);
    ...
    private boolean selectRecdAuditPlanReport(String AuditNumber, boolean isRecordSelected, int count) throws InterruptedException {
        WebElement table = driver.findElement(By.id("auditPlanDtlsGridId"));
        WebElement tableBody = table.findElement(By.tagName("tbody"));
        int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
        int totalNoOfRecords = 0;
        int noOfRecordsChecked = 0;
        if (perPageNoOfRecordsPresent > 0) {
            String a = driver.findElement(By.xpath("//*[@id=\"auditPlanDtlsGridId\"]/div/div[4]/div[2]/span")).getText();// For Ex: Showing 1-1 of 1
        ...
        if (perPageNoOfRecordsPresent > 0 && count == 0) {
            if ((totalNoOfRecords > 1) && ((AuditNumber == null) || ("".equalsIgnoreCase(AuditNumber)))) {
            	AuditNumber = driver.findElement(By.xpath("//*[@id=\"auditPlanDtlsGridId\"]/div/table/tbody/tr[1]/td[3]")).getText();//documentType
            } else if ((AuditNumber == null) || ("".equalsIgnoreCase(AuditNumber))) {
            	AuditNumber = driver.findElement(By.xpath("//*[@id=\"auditPlanDtlsGridId\"]/div/table/tbody/tr/td[3]")).getText();//documentType
            ...
            while (noOfRecordsChecked < totalNoOfRecords) {
                if (totalNoOfRecords > 1) {
                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
                        String AuditNumberSequence = driver.findElement(By.xpath("//*[@id=\"auditPlanDtlsGridId\"]/div/table/tbody/tr[ " + i + " ]/td[4]")).getText();//documentTypeName
                        if (AuditNumber.equalsIgnoreCase(AuditNumberSequence)) {
                            driver.findElement(By.xpath("//*[@id=\"auditPlanDtlsGridId\"]/div/table/tbody/tr[ " + i + " ]/td[4]")).click();
                    ...
                    String AuditNumberSequence = driver.findElement(By.xpath("//*[@id=\"auditPlanDtlsGridId\"]/div/table/tbody/tr/td[4]")).getText();
                    if (AuditNumber.equalsIgnoreCase(AuditNumberSequence)) {
                        driver.findElement(By.xpath("//*[@id=\"auditPlanDtlsGridId\"]/div/table/tbody/tr/td[4]")).click();
                        isRecordSelected = true;
                        break;
                    }
                }
                ...
                if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
                    driver.findElement(By.cssSelector("#auditPlanDtlsGridId > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();//next page in Document approve list
                    Thread.sleep(3000);
                    table = driver.findElement(By.id("auditPlanDtlsGridId"));//Document Tree approve table
