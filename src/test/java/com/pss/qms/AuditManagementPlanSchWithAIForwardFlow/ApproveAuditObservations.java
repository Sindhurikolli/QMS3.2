...
//		try {
			...
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ApproveAuditObservations"
					+ (new Random().nextInt()) + ".pdf";
			fos = new FileOutputStream(output);
			writer = PdfWriter.getInstance(document, fos);
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("ApproveAuditObservations", "PSS-QMS-006", "Pass");
...
//	}
	...
	private void methodToDoAuditObs() throws Exception {

		int count = 0;
		boolean isRecordSelected = false;
		String AMId = properties.getProperty("PLAN_NAME_IN_AI_PLAN_SCHEDULE");
		isRecordSelected = selectRecdAuditObs(AMId, isRecordSelected, count);
		Thread.sleep(1000);
		sno++;
...
//	         }
	...
	private boolean selectRecdAuditObs(String AMId, boolean isRecordSelected, int count) throws InterruptedException {
