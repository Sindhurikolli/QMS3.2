...
public class AdhocSchdReject extends AMLoginDetails {

	@Test
	public void toAdhocSchdReject() throws Exception {

//		try {

			document = new Document(PageSize.A4, 36, 36, 20, 20);
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "AdhocSchdReject"
					+ (new Random().nextInt()) + ".pdf";
			fos = new FileOutputStream(output);
			writer = PdfWriter.getInstance(document, fos);
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("AdhocSchdReject", "PSS-QMS-002", "Pass");
...
//	}
	...
	private void methodToDoPlanReject() throws Exception {

		Thread.sleep(3000);
		int count = 0;
		boolean isRecordSelected = false;
		String AuditId = properties.getProperty("ADHOC_NAME_IN_ADHOC_SHCEDULE_REJECT_FLOW");
		isRecordSelected = selectRecdPlanReject(AuditId, isRecordSelected, count);
		Thread.sleep(2000);
		sno++;
	...
	private boolean selectRecdPlanReject(String AuditId, boolean isRecordSelected, int count)
