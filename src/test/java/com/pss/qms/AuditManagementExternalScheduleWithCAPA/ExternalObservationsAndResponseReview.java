 ...
 * @author likhitha
 */
public class ExternalObservationsAndResponseReview extends AMLoginDetails {

	@Test
    public void ExternalObsReview() throws InterruptedException, IOException, DocumentException, Exception {
//		try {
			document = new Document(PageSize.A4, 36, 36, 20, 20);
		  	Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ExternalObservationsAndResponseReview"
            ...
            writer.open();
            HeaderFooterPageEvent event = new HeaderFooterPageEvent("ExternalAudit Review", "PSS-QMS-005",
					"Pass");
...
//      document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on External Menu",sno,false); 
      sno++;
      driver.findElement(By.xpath("//*[@id=\"externalAuditPerformMenus\"]/div/a")).click();
      document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on External Obs And Response Review Menu",sno,false); 
     methodToDoExternalAuditReview();
     document.close();
		writer.close();
...
//	}
    ...
    private void methodToDoExternalAuditReview() throws Exception {
        int count = 0;
        boolean isRecordSelected = false;
        String AuditId = properties.getProperty("EXTERNAL_NAME_EXTERNAL_SCHEDULE");
        isRecordSelected = selectRecdExternalAuditReview(AuditId, isRecordSelected, count);
        if (isRecordSelected) {
    ...
    private boolean selectRecdExternalAuditReview(String AuditId, boolean isRecordSelected, int count) throws InterruptedException {
        ...
        if (perPageNoOfRecordsPresent > 0 && count == 0) {
            if ((totalNoOfRecords > 1) && ((AuditId == null) || ("".equalsIgnoreCase(AuditId)))) {
            	AuditId = driver.findElement(By.xpath("//*[@id=\"auditsContainerInEnterAuditFindingsForm\"]/div/table/tbody/tr[1]/td[3]")).getText();//documentType
            } else if ((AuditId == null) || ("".equalsIgnoreCase(AuditId))) {
            	AuditId = driver.findElement(By.xpath("//*[@id=\"auditsContainerInEnterAuditFindingsForm\"]/div/table/tbody/tr/td[3]")).getText();//documentType
            ...
            while (noOfRecordsChecked < totalNoOfRecords) {
                if (totalNoOfRecords > 1) {
                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
                        String AuditNumberSequence = driver.findElement(By.xpath(".//*[@id='auditsContainerInEnterAuditFindingsForm']/div/table/tbody/tr[ " + i + "]/td[3]")).getText();//documentTypeName
                    ...
                    String AuditNumberSequence = driver.findElement(By.xpath("//*[@id=\"auditsContainerInEnterAuditFindingsForm\"]/div/table/tbody/tr/td[3]")).getText();
                    if (AuditId.equalsIgnoreCase(AuditNumberSequence)) {
