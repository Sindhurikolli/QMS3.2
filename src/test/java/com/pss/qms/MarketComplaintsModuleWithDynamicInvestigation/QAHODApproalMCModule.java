...
public class QAHODApproalMCModule extends MCLoginDetails {

	@Test
	public void QAHODApprovalMCModule1() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "QAHODApprovalMCModule" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("QAHODApprovalMCModule", "PSS-QMS-014", "Pass");
...
//				false);
		...
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on HOD Review menu", sno, false);
		Thread.sleep(2000);
		WebDriverWait wait = new WebDriverWait(driver, 300);
		wait.until(ExpectedConditions.invisibilityOf(
				driver.findElement(By.cssSelector("#mcReviewTableContailner > div > div.jtable-busy-message"))));
		methodToDoQAHODApprovalerMC();
		document.close();
		writer.close();
	...
	private void methodToDoQAHODApprovalerMC() throws Exception {
		...
		isRecordSelected = selectRecdQAHODApprovalerMC(MarketComplaintsId, isRecordSelected, count);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select the record", sno, false);
		Thread.sleep(4000);
		if (isRecordSelected) {
			sno++;
			driver.findElement(By.id("mcAddSuppDocsInQAFinal")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Add button", sno, false);
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("uploadSuppDocQAFinal_1")).sendKeys(properties.getProperty("Document-2"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Upload the document", sno, false);
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("commentsInMcQaFinal")).sendKeys(properties.getProperty("MC_994"));
	...
	private boolean selectRecdQAHODApprovalerMC(String MCNumber, boolean isRecordSelected, int count)
		...
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"mcReviewTableContailner\"]/div/div[4]/div[2]/span")).getText();// For
			//*[@id="mcReviewTableContailner"]/div/table/tbody/tr[1]/td[3]
																														// 1
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((MCNumber == null) || ("".equalsIgnoreCase(MCNumber)))) {
				MCNumber = driver.findElement(By.xpath("//*[@id=\"mcReviewTableContailner\"]/div/table/tbody/tr[1]/td[3]"))
				...
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String capaNumberSequence = driver
								.findElement(
										By.xpath(".//*[@id='mcReviewTableContailner']/div/table/tbody/tr[ " + i + " ]/td[3]"))
					...
					Thread.sleep(3000);
					String capaNumberSequence = driver
							.findElement(By.xpath("//*[@id=\"mcReviewTableContailner\"]/div/table/tbody/tr/td[3]")).getText();
					Thread.sleep(3000);
					if (MCNumber.equalsIgnoreCase(capaNumberSequence)) {
						Thread.sleep(3000);
						driver.findElement(By.xpath("//*[@id=\"mcReviewTableContailner\"]/div/table/tbody/tr/td[51]/button"))
								.click();
						Thread.sleep(3000);
						isRecordSelected = true;
						break;
					}
				}//*[@id="mcReviewTableContailner"]/div/table/tbody/tr[1]/td[51]/button
...
//					driver.findElement(By.cssSelector("#mcReviewTableContailner > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();// next page in Document approve list
					...
					Thread.sleep(5000);
					Helper.clickElement(driver, By.cssSelector("#mcReviewTableContailner > div > div.jtable-busy-message[style='display: none;']"));
