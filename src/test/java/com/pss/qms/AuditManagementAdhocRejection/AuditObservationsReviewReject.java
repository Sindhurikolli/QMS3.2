...
public class AuditObservationsReviewReject extends AMLoginDetails {
	@Test

	public void toAuditObservationsReviewReject() throws Exception {

//		try {

			document = new Document(PageSize.A4, 36, 36, 20, 20);
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReviewAuditObservationsAM"
					+ (new Random().nextInt()) + ".pdf";
			fos = new FileOutputStream(output);
			writer = PdfWriter.getInstance(document, fos);
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("AuditObservationsReview Reject", "PSS-QMS-007",
					"Pass");
...
//	}
	...
	private void methodToDoReviewAuditReject() throws Exception {

		int count = 0;
		boolean isRecordSelected = false;
		String AMId = properties.getProperty("ADHOC_NAME_IN_ADHOC_SHCEDULE_REJECT_FLOW");
		isRecordSelected = selectRecdAuditObservationsReviewReject(AMId, isRecordSelected, count);
		Thread.sleep(2000);
		sno++;
		...
		if (isRecordSelected) {
			...
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("commentsInAmReviewAuditFindingForm")).sendKeys(properties.getProperty("AM_1500"));
	...
	private boolean selectRecdAuditObservationsReviewReject(String AMId, boolean isRecordSelected, int count)
