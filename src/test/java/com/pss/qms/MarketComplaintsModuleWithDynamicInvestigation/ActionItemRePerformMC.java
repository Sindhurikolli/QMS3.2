...
public class ActionItemRePerformMC extends MCLoginDetails {

	@Test
	public void ActionItemRePerformMCMethod() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ActionItemRePerformMC"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("ActionItemRePerformMC", "PSS-QMS-016", "Pass");
...
//    Thread.sleep(20000);
		...
		wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("#actItmRejectTable > div > div.jtable-busy-message[style='display: none;']")));
	...
	private void methodToDoAIPerformMC() throws Exception {
		...
		if (isRecordSelected) {
			...
			WebElement element = driver.findElement(By.id("mcAddSuppDocsInMcActItem"));
			...
			sno++;
			Thread.sleep(5000);
			Select Approver=new Select(driver.findElement(By.id("appFromInRejectForm")));
			Approver.selectByVisibleText(properties.getProperty("AI_Approver"));
			Thread.sleep(2000);
			driver.findElement(By.id("commentsInRejectForm")).sendKeys(properties.getProperty("MC_994"));
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					...
					Thread.sleep(3000);
					String capaNumberSequence = driver
							.findElement(By.xpath("//*[@id=\"actItmRejectTable\"]/div/table/tbody/tr/td[3]"))
							.getText();
					Thread.sleep(3000);
					if (MarketComplaintsId.equalsIgnoreCase(capaNumberSequence)) {
						Thread.sleep(3000);
						driver.findElement(By.xpath("//*[@id=\"actItmRejectTable\"]/div/table/tbody/tr/td[3]"))
								.click();
						Thread.sleep(3000);
						isRecordSelected = true;
						break;
