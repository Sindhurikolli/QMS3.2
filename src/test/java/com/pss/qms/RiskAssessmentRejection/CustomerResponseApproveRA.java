...
public class CustomerResponseApproveRA extends RALoginDetails {

	@Test
	public void CustomerResponseApproveRAMethod() throws Exception {

		document = new Document(PageSize.A4, 36, 36, 20, 20);
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CustomerResponseApproveRA"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("CustomerResponseApproveRA", "PSS-QMS-001", "Pass");
...
//         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on MyActivities Tab",sno,false); 
		...
		methodToDoCustomerResponseApproveRA();
		Thread.sleep(3000);
		document.close();
		writer.close();
	...
	private void methodToDoCustomerResponseApproveRA() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("responseInRaNotifyCustWindow"))
					.sendKeys(properties.getProperty("RA_2000"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter The Comments", sno, false);
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("custRevDocInRaNotifyCustWindow")).sendKeys(properties.getProperty("Document-3"));
