...
public class CQAApproveRejectRA extends RALoginDetails {

	@Test
	public void CQAApproverRA() throws Exception {

		document = new Document(PageSize.A4, 36, 36, 20, 20);
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CQAApproveRejectRA"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("CQAApproveRejectRA", "PSS-QMS-001", "Pass");
		...
		document.add(new Paragraph(
				sno + "." + "Enter the username, password and select Risk Assessment Module then click on login button"
...
//         jse.executeScript("arguments[0].scrollIntoView(true);", element);
	...
	private void methodToDoCQAApproverRA() throws Exception {

		Thread.sleep(5000);
		int count = 0;
		boolean isRecordSelected = false;
		String RASequence = properties.getProperty("RA_NUMBER");
		isRecordSelected = selectRecdCQAApproverRA(RASequence, isRecordSelected, count);
		Thread.sleep(3000);
		if (isRecordSelected) {
			sno++;
			driver.findElement(By.id("commentsInCQaCcReview")).sendKeys(properties.getProperty("RA_2000"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter The Comments", sno, false);
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("rejectActivityInCheckInCQAApp")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On Reject Button", sno, false);
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("submitButtonIdInRaCqa")).click();
	...
	private boolean selectRecdCQAApproverRA(String RANumber, boolean isRecordSelected, int count)
