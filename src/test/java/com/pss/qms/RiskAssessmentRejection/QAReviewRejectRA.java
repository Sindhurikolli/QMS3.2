...
public class QAReviewRejectRA extends RALoginDetails {

	@Test
	public void QAReviewRejectRA() throws Exception {

		document = new Document(PageSize.A4, 36, 36, 20, 20);
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "QAReviewRejectRA" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("QAReviewRejectRA", "PSS-QMS-010", "Pass");
...
//         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on MyActivities Tab",sno,false);  
	...
	private void methodToDoQAReviewRejectRA() throws Exception {
		...
		isRecordSelected = selectRecdQAReviewRejectRA(RASequence, isRecordSelected, count);
		Thread.sleep(8000);
		if (isRecordSelected) {
			sno++;
			...
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("rejectActivityInCheckInRaQaPriRev")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On Reject Button", sno, false);
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("rejectReasonInRaQaPrimaryReview")).sendKeys(properties.getProperty("RA_2000"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Reason For Reject", sno, false);
			Thread.sleep(1000);
			sno++;
	...
	private boolean selectRecdQAReviewRejectRA(String RANumber, boolean isRecordSelected, int count)
