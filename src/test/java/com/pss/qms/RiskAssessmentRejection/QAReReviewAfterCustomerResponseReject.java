...
public class QAReReviewAfterCustomerResponseReject extends RALoginDetails {

	@Test
	public void QAReReviewAfterCustomerResponseRejectMethod() throws Exception {

		document = new Document(PageSize.A4, 36, 36, 20, 20);
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "QAReReviewAfterCustomerResponseReject"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("QAReReviewAfterCustomerResponseReject", "PSS-QMS-001", "Pass");
...
//         document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on MyActivities Tab",sno,false);  
		...
		methodToDoQAReReviewAfterCustomerResponseReject();
		Thread.sleep(3000);
		document.close();
		writer.close();
	...
	private void methodToDoQAReReviewAfterCustomerResponseReject() throws Exception {

		Thread.sleep(5000);
		int count = 0;
		boolean isRecordSelected = false;
		String RASequence = properties.getProperty("RA_NUMBER");
		isRecordSelected = selectRecdQAReReviewAfterApproveReject(RASequence, isRecordSelected, count);
		Thread.sleep(4000);
		if (isRecordSelected) {
			sno++;
			...
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("commentsInRiskAssQaReview")).clear();
			driver.findElement(By.id("commentsInRiskAssQaReview")).sendKeys(properties.getProperty("RA_2000"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Comments", sno, false);
			Thread.sleep(1000);
			sno++;
			driver.findElement(By.id("reviewCompletedInPaQaReview")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On QAApprover RadioButton", sno,
					false);
			Thread.sleep(1000);
//            JavascriptExecutor jse = (JavascriptExecutor) driver;
//            WebElement element = driver.findElement(By.id("submitBtnRaQaPrimaryReview"));
...
//            Thread.sleep(1000);
	...
	private boolean selectRecdQAReReviewAfterApproveReject(String RANumber, boolean isRecordSelected, int count)
