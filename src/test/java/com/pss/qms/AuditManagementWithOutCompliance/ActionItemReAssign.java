...
public class ActionItemReAssign extends AMLoginDetails {
    
    
	   @Test
	    public void ActionItemReAssignInAM()  throws InterruptedException, IOException, DocumentException, Exception {
//			try {
				
			document = new Document(PageSize.A4, 36, 36, 20, 20);
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		    document = new Document();
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ActionItemReAssign"
							+ (new Random().nextInt()) + ".pdf";
		    fos = new FileOutputStream(output);
     	writer = PdfWriter.getInstance(document, fos);
				
					writer.open();
					HeaderFooterPageEvent event = new HeaderFooterPageEvent("ActionItem ReAssign", "PSS-QMS-010",
							"Pass");
...
//		}
	    ...
	    private void methodToDoActionItemReAssign() throws Exception {
	        int count = 0;
	        boolean isRecordSelected = false;
	        String ActionId = properties.getProperty("AI_NUMBER");
	        String ActionItemNumber = ActionId;
	        isRecordSelected = selectRecdActionItemReAssign(ActionItemNumber,isRecordSelected, count);
	        Thread.sleep(3000);
	        if (isRecordSelected) {
	        	sno++;
	        	driver.findElement(By.id("ApprCheckInAmActItmReassign")).click();
...
//	            driver.findElement(By.id("suppDocumentInAmActItmPerform")).sendKeys(properties.getProperty("Document-1"));
	            driver.findElement(By.id("selectAppApprovalInCcQaPriReview1")).click();
	          ...
	          document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click On Select Button",sno,false);
	          sno++;
	            driver.findElement(By.id("reasonInAmActItmReassign")).sendKeys(properties.getProperty("AUDIT_COMMENTS_1500"));
...
//	            document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter The Comments "+"  PSS-QMS-SS-032",sno,false);
	             driver.findElement(By.id("amActItmReassignSubmitBtn")).click();
		...
		private boolean selectRecdActionItemReAssign(String ActionItemNumber, boolean isRecordSelected, int count) throws InterruptedException {
	        WebElement table = driver.findElement(By.id("actItmReassignTable"));
	        WebElement tableBody = table.findElement(By.tagName("tbody"));
	        int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
	        int totalNoOfRecords = 0;
	        int noOfRecordsChecked = 0;
	        if (perPageNoOfRecordsPresent > 0) {
	            String a = driver.findElement(By.xpath("//*[@id=\"actItmReassignTable\"]/div/div[4]/div[2]/span")).getText();// For Ex: Showing 1-1 of 1
	            String[] parts = a.split(" of ");
	            try {
	                totalNoOfRecords = Integer.parseInt(parts[1].trim());
	            } catch (Exception e) {
	        ...
	        if (perPageNoOfRecordsPresent > 0 && count == 0) {
	            if ((totalNoOfRecords > 1) && ((ActionItemNumber == null) || ("".equalsIgnoreCase(ActionItemNumber)))) {
	            	ActionItemNumber = driver.findElement(By.xpath("//*[@id=\"actItmReassignTable\"]/div/table/tbody/tr[1]/td[3]")).getText();//documentType
	            } else if ((ActionItemNumber == null) || ("".equalsIgnoreCase(ActionItemNumber))) {
	            	ActionItemNumber = driver.findElement(By.xpath("//*[@id=\"actItmReassignTable\"]/div/table/tbody/tr/td[3]")).getText();//documentType
	        ...
	        if (perPageNoOfRecordsPresent > 0) {
	            while (noOfRecordsChecked < totalNoOfRecords) {
	                if (totalNoOfRecords > 1) {
	                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
	                        String AMNumberSequence = driver.findElement(By.xpath("//*[@id=\"actItmReassignTable\"]/div/table/tbody/tr[ " + i + " ]/td[3]")).getText();//documentTypeName
	                        if (ActionItemNumber.equalsIgnoreCase(AMNumberSequence)) {
	                            driver.findElement(By.xpath("//*[@id=\"actItmReassignTable\"]/div/table/tbody/tr[ " + i + " ]/td[3]")).click();
	                    ...
	                    String AMNumberSequence = driver.findElement(By.xpath("//*[@id=\"actItmReassignTable\"]/div/table/tbody/tr/td[3]")).getText();
	                    if (ActionItemNumber.equalsIgnoreCase(AMNumberSequence)) {
	                        driver.findElement(By.xpath("//*[@id=\"actItmReassignTable\"]/div/table/tbody/tr/td[3]")).click();
	                        isRecordSelected = true;
	                        break;
	                    }
	                }
	                ...
	                if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
	                    driver.findElement(By.cssSelector("#actItmReassignTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();//next page in Document approve list
	                    Thread.sleep(3000);
	                    table = driver.findElement(By.id("actItmReassignTable"));//Document Tree approve table
	                    tableBody = table.findElement(By.tagName("tbody"));
	                    perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
	                }
	            }
