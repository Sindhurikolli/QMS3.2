...
public class RAApproval extends RALoginDetails {

	@Test
	public void RAApproval() throws Exception {
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("RAApproval", "PSS-QMS-002", "Pass");
...
//					false);
		...
		wait1.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("a[href='raRiskAsmtAppPage.do']")));
		driver.findElement(By.cssSelector("a[href='raRiskAsmtAppPage.do']")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Risk Identification Approval Menu",
				sno, false);
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						if (RANumber.equalsIgnoreCase(RANumberSequence)) {
							...
							WebElement ele = driver.findElement(
									By.xpath("//*[@id=\"riskAnalysisAppTableContainer\"]/div/table/tbody/tr[ " + i
											+ " ]/td[12]/button"));
					...
					if (RANumber.equalsIgnoreCase(RANumberSequence)) {
						driver.findElement(
								By.xpath("//*[@id=\"riskAnalysisAppTableContainer\"]/div/table/tbody/tr/td[12]/button"))
								.click();
						isRecordSelected = true;
						break;
					}
