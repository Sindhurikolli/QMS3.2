 ...
 * @author likhitha
 */
public class RegulatoryGroups extends RegistrationLoginDetails {
    
    @Test
    public void RegGroupRegistration() throws InterruptedException {
    		Thread.sleep(2000);
           ...
           Thread.sleep(5000);
           driver.findElement(By.cssSelector("a[href='createRegGroups.do']")).click();
           Thread.sleep(5000);
           todoRegGroupRegistration();
      }
           public void todoRegGroupRegistration() throws InterruptedException {    
        ...
        Thread.sleep(4000);
          boolean isRecordSelectedForPlant = false;
            String PlantFirstName = properties.getProperty("RAPlantName");
            int count3=0;
            isRecordSelectedForPlant=selectingThePlant(PlantFirstName,isRecordSelectedForPlant,count3);
            Thread.sleep(3000);
            if(isRecordSelectedForPlant)
            {
        driver.findElement(By.id("selectBtnInPlantSelWin")).click();
        Thread.sleep(5000);
        Select UG = new Select(driver.findElement(By.id("userGroupInCreateRegGroup")));
        UG.selectByVisibleText(properties.getProperty("UserGroupforRA"));
        Thread.sleep(3000);
    ...
    private boolean selectingThePlant(String PlantFirstName, boolean isRecordSelectedForPlant, int count3) throws InterruptedException {
            ...
            while (noOfRecordsChecked < totalNoOfRecords) {
                if (perPageNoOfRecordsPresent > 1) {
                    for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
                        String cftReviewerFullName = driver.findElement(By.xpath("//*[@id=\"plantCodeSelGrid\"]/div/table/tbody/tr[ " + i + " ]/td[3]")).getText();//documentTypeName
                        System.out.println("cftReviewerFullName: "+cftReviewerFullName);
                        if (PlantFirstName.equalsIgnoreCase(cftReviewerFullName)) {
                            driver.findElement(By.xpath("//*[@id=\"plantCodeSelGrid\"]/div/table/tbody/tr[ " + i + " ]/td[3]")).click();
                            isRecordSelectedForPlant = true;
                            break;
                        }
                    }
                   
                } else {
                    String cftReviewerFullName = driver.findElement(By.xpath("//*[@id=\"plantCodeSelGrid\"]/div/table/tbody/tr/td[3]")).getText();
                    if (PlantFirstName.equalsIgnoreCase(cftReviewerFullName)) {
                ...
                if ((!isRecordSelectedForPlant) && (noOfRecordsChecked < totalNoOfRecords)) {
        ...
        return isRecordSelectedForPlant;
    }
    @AfterMethod
