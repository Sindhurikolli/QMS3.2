...
public class ActionItemRePerform extends AMLoginDetails {

	@Test
	public void toActionItemRePerform() throws Exception {

//		try {

			document = new Document(PageSize.A4, 36, 36, 20, 20);
			Font font = new Font(Font.FontFamily.TIMES_ROMAN);
			output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ActionItemRePerform"
					+ (new Random().nextInt()) + ".pdf";
			fos = new FileOutputStream(output);
			writer = PdfWriter.getInstance(document, fos);
			writer.open();
			HeaderFooterPageEvent event = new HeaderFooterPageEvent("ActionItemRePerform", "PSS-QMS-020", "Pass");
...
//	        document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Approvals Menu",sno,false);
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.cssSelector("a[href='amActionItemRejectPage.do']")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on ActionItem Reperform Menu",
					sno, false);
			Helper.waitLoadRecords(driver, By.cssSelector("#actItmRejectTable > div > div.jtable-busy-message[style='display: none;']"));
...
//	}
	...
	private void methodToDoAuditeeResponseApprove() throws Exception {
		...
		String AMId = properties.getProperty("PLAN_NAME_IN_AI_PLAN_SCHEDULE_AINO");
...
//	         ApproveFrom.selectByIndex(2);
//	         Thread.sleep(2000);
//	         driver.findElement(By.id("commentsInAmPerform")).sendKeys(properties.getProperty("AM_1500"));
//	         Thread.sleep(2000);
//	         driver.findElement(By.xpath("//*[@id=\"attatchmentsJtableInAmActItemPerform\"]/div/div[3]/div[2]/span/span[2]")).click();
//	         Thread.sleep(2000);
	...
	private boolean selectRecdAuditeeResponseApprove(String AMId, boolean isRecordSelected, int count)
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((AMId == null) || ("".equalsIgnoreCase(AMId)))) {
				AMId = driver.findElement(By.xpath("//*[@id=\"actItmRejectTable\"]/div/table/tbody/tr[1]/td[3]"))
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						Helper.scrollElement(driver, By.xpath("//*[@id=\"actItmRejectTable\"]/div/table/tbody/tr[ " + i + " ]/td[3]"));
						String AMNumberSequence = driver.findElement(By.xpath("//*[@id=\"actItmRejectTable\"]/div/table/tbody/tr[ " + i + " ]/td[3]")).getText();// documentTypeName
						if (AMId.equalsIgnoreCase(AMNumberSequence)) {
							Helper.clickElement(driver, By.xpath("//*[@id=\"actItmRejectTable\"]/div/table/tbody/tr[ " + i + " ]/td[3]"));
//							driver.findElement(By.xpath("//*[@id=\"actItmRejectTable\"]/div/table/tbody/tr[ " + i + " ]/td[3]")).click();
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					Helper.clickElement(driver, By.cssSelector("#actItmRejectTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"));
